...
def setup_redis():
    ...
    REDIS_CLIENT_CP_DATA = redis.Redis(
        host=redis_host, port=redis_port, decode_responses=True, db=redis_db_idx_cp_data  # optional, strings not bytes
    )
    REDIS_CLIENT_GENERAL = redis.Redis(
        host=redis_host, port=redis_port, decode_responses=True, db=redis_db_idx_last_notification_to_badge  # optional, strings not bytes
    )
